...
from llm import (
    EXAMPLE_POLICY_PATH,
    INITIAL_POP_TEMPLATE_PATH,
    MUTATION_TEMPLATE_PATH,
    POLICY_FORMAT_GUIDANCE_PATH,
)
...
def run_flappy_game(policy: dict | None = None, interactive: bool = False, max_frames: int = 10000, render: bool = True, return_fitness: bool = False) -> int | None:
    """
    Runs a Flappy Bird game session, either interactively or with an AI policy.
    Returns the fitness score if return_fitness is True, else None.
    """
    ...
    while running and not game_over:  # Main game loop
        ...
        if (render):  # Only process events if rendering to prevent issues in headless mode
            for event in pygame.event.get():
                ...
                if (interactive and event.type == pygame.KEYDOWN and event.key == pygame.K_SPACE):
                    action_to_take = 1
            if not running:
                break  # Exit if QUIT event handled
        ...
        if render and screen:  # Rendering block
            ...
            pygame.draw.rect(
                screen,
                pipe_color,
                (game_instance.pipe_x, 0, game_instance.pipe_width, uig_h),
            )
            pygame.draw.rect(
                screen,
                pipe_color,
                (
                    game_instance.pipe_x,
                    lig_y,
                    game_instance.pipe_width,
                    FlappyBirdGame.GAME_SCREEN_HEIGHT - lig_y,
                ),
            )
            bird_color = (255, 215, 0)
            bird_rx = 60
            bird_ryc = int((game_instance.bird_y / FlappyBirdGame.BIRD_Y_MAX) *
                           FlappyBirdGame.GAME_BIRD_LOGICAL_Y_TO_PIXEL_Y_EFFECTIVE_RANGE)
            pygame.draw.circle(
                screen,
                bird_color,
                (bird_rx, bird_ryc),
                FlappyBirdGame.GAME_BIRD_PIXEL_RADIUS,
            )
        ...
        if game_instance.frames_survived >= max_frames:
            game_over = True
...
def play_game_with_policy(policy_to_play: dict | None = None) -> None:
    """
    Loads and plays a game using the provided or saved policy.
    """
...
def play_game_interactive() -> None:
    """
    Allows a human to play the game interactively.
    """
...
TITLE_Y = 80
MENU_ITEM_Y_START = 200
MENU_ITEM_Y_STEP = 60
PROMPT_FILES = {
    POLICY_FORMAT_GUIDANCE_PATH: "Define policy format guidance.",
    EXAMPLE_POLICY_PATH: '{ "default": "flap" }',
...
def main_menu() -> None:
    """
    Display the main menu, handle user input, and manage mode switching for the Flappy Evo application.
    """
    pygame.init()
    WIDTH, HEIGHT = MENU_WIDTH, FlappyBirdGame.GAME_SCREEN_HEIGHT
    screen = pygame.display.set_mode((WIDTH, HEIGHT))
    ...
    policy_file_valid = False

    os.makedirs("prompts", exist_ok=True)
    for p_file, p_content in PROMPT_FILES.items():
        if not os.path.exists(p_file):
            try:
                ...
                print(f"[ERROR] Could not create placeholder {p_file}: {e}")

    def get_menu_options_list() -> list[tuple[str, str]]:
        """
        Returns the list of menu options as (label, action_key) tuples, depending on policy file state.
        """
            ...
            try:
                with open("best_policy.json", "r", encoding="utf-8") as f:
                    ...
                    if validate_policy(loaded_p):  # Critical validation
                        ...
                        policy_file_valid = True
                        print("[INFO] Menu: Loaded valid best_policy.json.")
                    else:
                        print("[INFO] Menu: best_policy.json found but invalid.")
                        policy_file_valid = False
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_UP:
                    selected_index = ((selected_index - 1 + len(menu_items)) % len(menu_items) if menu_items else 0)
                elif event.key == pygame.K_DOWN:
                    selected_index = ((selected_index + 1) % len(menu_items) if menu_items else 0)
                elif event.key == pygame.K_RETURN and menu_items:
                    action_key = menu_items[selected_index][1]
                    if action_key == "play_best":
                        if current_best_policy and validate_policy(current_best_policy):
                            play_game_with_policy(current_best_policy)
                        else:
                            print("[Menu] No valid best policy to play.")
                    elif action_key == "train":
                        newly_trained = train_policy()
                        if newly_trained and validate_policy(newly_trained):
                            current_best_policy = newly_trained
                        else:
                            current_best_policy = None
                        policy_file_checked = False
                    elif action_key == "play_human":
                        play_game_interactive()
                    elif action_key == "quit":
                        menu_running = False
                    menu_items = get_menu_options_list()
                    selected_index = (min(selected_index, len(menu_items) - 1) if menu_items else 0)
        screen.fill(MENU_BG_COLOR)
        title_surf = font_large.render("Flappy Evo", True, TITLE_COLOR)
        screen.blit(title_surf, title_surf.get_rect(center=(WIDTH // 2, TITLE_Y)))
        for i, (text, _) in enumerate(menu_items):
            color = SELECTED_COLOR if i == selected_index else UNSELECTED_COLOR
            item_surf = font_small.render(text, True, color)
            screen.blit(item_surf, item_surf.get_rect(center=(WIDTH // 2, MENU_ITEM_Y_START + i * MENU_ITEM_Y_STEP)))
        pygame.display.flip()
        clock.tick(30)
    pygame.quit()
