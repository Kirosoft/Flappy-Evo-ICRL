...
LLM_PROMPT_POLICY_FORMAT_GUIDANCE = read_file_content(POLICY_FORMAT_GUIDANCE_PATH,
                                                      "Error: Policy format guidance missing.")
...
# --- Constants ---
FAILED_POLICY_CACHE_SIZE = 2  # Number of failed policies to include in LLM prompt

def call_ollama_blocking(prompt: str, temperature: float) -> tuple[dict | None, str]:
    """
    Calls the Ollama LLM endpoint with the given prompt and temperature.
    ...
    try:
        ...
        log_failed_llm_attempt(prompt, None, f"Timeout {LLM_TIMEOUT}s")
        return None, "Request Timed Out"
    except requests.exceptions.RequestException as e:
        log_failed_llm_attempt(prompt, raw_response_text_for_log, f"Ollama connection error: {e}")
        return None, f"Ollama Connection Error: {raw_response_text_for_log}"
    except json.JSONDecodeError as e:
        log_failed_llm_attempt(
            prompt,
            json_string if "json_string" in locals() else raw_response_text_for_log,
            f"JSON decode error: {e}",
        )
        return None, (json_string if "json_string" in locals() else raw_response_text_for_log)
    except Exception as e:
        log_failed_llm_attempt(prompt, raw_response_text_for_log, f"Unexpected error: {e}")
        return None, f"Unexpected Error: {raw_response_text_for_log}"


def call_ollama_blocking_for_thread(prompt: str, temperature: float, result_container: dict, error_container: dict, raw_response_container: dict) -> None:
    """
    Thread-safe wrapper for call_ollama_blocking. Populates result_container or error_container.
    """
...
def llm_generate_initial_population_prompt_str(n: int = 5, failed_policies: list | None = None) -> str:
    """
    Generates the initial population prompt string for the LLM, including up to FAILED_POLICY_CACHE_SIZE failed policies.
    """
    failed_policies = failed_policies or []
    failed_json_str = (json.dumps(failed_policies[-FAILED_POLICY_CACHE_SIZE:], indent=2) if failed_policies else "[]")
    prompt = LLM_INITIAL_POP_TEMPLATE.format(
        n=n,
        policy_format_guidance=LLM_PROMPT_POLICY_FORMAT_GUIDANCE,
        failed_json_str=failed_json_str,
        example_policy_json_str=LLM_EXAMPLE_POLICY_JSON_STR,
    )
    return prompt


def llm_intelligent_mutation(policy: dict) -> dict | None:
    """
    Uses the LLM to intelligently mutate a policy. Returns the mutated policy dict if valid, else None.
    """
    print(f"[LLM Call - Mutate]: Asking '{OLLAMA_MODEL}'...")
    policy_to_mutate_json_str = json.dumps(policy, indent=2)
    prompt = LLM_MUTATION_TEMPLATE.format(
        policy_to_mutate_json_str=policy_to_mutate_json_str,
        policy_format_guidance=LLM_PROMPT_POLICY_FORMAT_GUIDANCE,
        example_policy_json_str=LLM_EXAMPLE_POLICY_JSON_STR,
    )
    parsed_data, raw_response = call_ollama_blocking(prompt, LLM_TEMPERATURE_MUTATION)
    if parsed_data is None:
        print(f"[LLM Failure - Mutate] Invalid/No response. See '{FAILED_LLM_LOG_FILE}'.")
        return None
    mutated_policy = normalize_policy(parsed_data)
    if mutated_policy and validate_policy(mutated_policy):
        print("[LLM Success - Mutate] Valid mutation.")
        return mutated_policy
    else:
        print(f"[LLM Failure - Mutate] Validation fail. Norm: {mutated_policy}. See '{FAILED_LLM_LOG_FILE}'.")
        log_failed_llm_attempt(
            prompt,
            raw_response or "No raw response",
            f"Validation failed. Norm: {mutated_policy}",
        )
        return None
