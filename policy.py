...
def train_policy() -> dict | None:
    """
    Trains a policy using evolutionary and LLM-guided methods. Returns the best policy found, or None on failure.
    """
    ...
    loading_text_surf = font_plot.render(
        "LLM: Generating initial population (this will freeze GUI)...",
        True,
        (255, 255, 255),
    )
    ...
    prompt_for_initial_pop = llm_generate_initial_population_prompt_str(num_to_request_from_llm,
                                                                        failed_policies_for_llm)
    ...
    if llm_data is None:  # call_ollama_blocking handles logging the specific error
        print(
            f"[LLM Error/Warning - Initial Pop]: LLM call failed or returned no usable data. Raw: {raw_llm_response_text[:200] if raw_llm_response_text else 'N/A'}"
        )
    ...
    if normalized_policies and isinstance(normalized_policies, list):
        ...
        if len(population) == 0 and llm_data is not None:
            ...
            if raw_llm_response_text:
                log_failed_llm_attempt(
                    prompt_for_initial_pop,
                    raw_llm_response_text,
                    f"Initial Pop - All items failed normalize/validate. Normalized output: {normalized_policies}",
                )
    else:
        print(
            f"  Normalization of initial pop data failed or returned unexpected type. Norm_Output: {normalized_policies}"
        )
    ...
    for gen_counter in range(NUM_GENERATIONS):  # Use gen_counter to reflect actual EA cycles
        ...
        if not pop_with_fitness:
            ...
            if not population:
                print("[CRITICAL ERROR] Failed to re-seed random population. Stopping.")
                break
        ...
        if len(failed_policies_for_llm) > 5:
            failed_policies_for_llm = failed_policies_for_llm[-5:]
        ...
        pygame.draw.line(
            ...
            (plot_margin, plot_margin + graph_height),
            2,
        )
        pygame.draw.line(
            plot_screen,
            (200, 200, 200),
            (plot_margin, plot_margin + graph_height),
            (plot_margin + graph_width, plot_margin + graph_height),
            2,
        )
        if fitness_history:
            max_f_label = max(fitness_history, default=1.0)
            min_f_label = min(fitness_history, default=0.0)
            max_surf = font_plot_small.render(f"{max_f_label:.0f}", True, (200, 200, 200))
            plot_screen.blit(max_surf, (plot_margin - 40, plot_margin - 7))
            min_surf = font_plot_small.render(f"{min_f_label:.0f}", True, (200, 200, 200))
            plot_screen.blit(min_surf, (plot_margin - 40, plot_margin + graph_height - 7))
        if len(fitness_history) > 1:
            max_f_hist = max(fitness_history, default=1.0)
            min_f_hist = min(fitness_history, default=0.0)
            f_range = max_f_hist - min_f_hist
            f_range = f_range if f_range > 0 else 1.0
            points = []
            num_gen_plotted = len(fitness_history)
            for i_fh, f_val in enumerate(fitness_history):
                x_coord = plot_margin + int(
                    (i_fh / max(1, num_gen_plotted - 1)) * graph_width)  # Scale based on plotted points
                y_coord = (plot_margin + graph_height - int(((f_val - min_f_hist) / f_range) * graph_height))
                points.append((x_coord, y_coord))
            if len(points) > 1:
                pygame.draw.lines(plot_screen, (255, 215, 0), False, points, 2)
    ...
    if overall_best_policy:
        ...
        try:
            with open("best_policy.json", "w", encoding="utf-8") as f:
                json.dump(overall_best_policy, f, indent=2)
            print("Saved overall best policy to best_policy.json")
        except Exception as e:
            print(f"Error saving overall_best_policy.json: {e}")
    else:
...
def normalize_policy(policy_data_input, expect_list: bool = False) -> list | dict | None:
    """
    ...
    def _normalize_and_correct_single_policy_dict(llm_policy_dict_item):
        ...
        if (isinstance(original_default_action, str)
                and original_default_action.lower() == "do_no'thing"):  # Specific typo fix
...
def validate_policy(policy: dict) -> bool:
    """
    Validates a policy dict for correct structure and allowed values.
        ...
        if key != "default":
            ...
            for p_idx, p_val_str in enumerate(parts):
                ...
                if p_val_str.startswith("pos:"):
                    ...
                    if val in ["above", "aligned", "below"]:
                        valid_part_syntax = True
                        current_part_prefix = "pos:"
                elif p_val_str.startswith("dist:"):
                    val = p_val_str.split(":", 1)[1]
                    if val in ["far", "medium", "close"]:
                        valid_part_syntax = True
                        current_part_prefix = "dist:"
                elif p_val_str.startswith("velo:"):
                    val = p_val_str.split(":", 1)[1]
                    if val in ["rising", "stable", "falling"]:
                        valid_part_syntax = True
                        current_part_prefix = "velo:"
...
def get_action_from_policy(policy: dict, full_coded_state: str, action_map: dict) -> int:
    """
    Determines the action to take for a given state using the policy dict and action map.
    Returns the numeric action.
    """
...
def generate_random_policy() -> dict:
    """
    Generates a random policy dict for initialization or mutation.
    """
    policy = {"default": random.choice(["flap", "do_nothing"])}
    for _ in range(random.randint(0, 2)):
        key = generate_random_state_key(random.randint(1, 3))
        if key and key not in policy:
            policy[key] = random.choice(["flap", "do_nothing"])
    return policy


def generate_random_state_key(num_parts: int) -> str:
    """
    Generates a random state key with the specified number of parts (1-3).
    """
    ...
    if not 0 < num_parts <= len(available_types):
        num_parts = random.randint(1, len(available_types))
